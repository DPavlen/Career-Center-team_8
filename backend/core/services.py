...
def candidate_resume_pdf(candidate_id):
    ...
    experience_in_candidate = ExperienceDetailedInCandidate.objects.filter(
        candidate=candidate
    )
    education_in_candidate = EducationInCandidate.objects.filter(
        candidate=candidate
    )
    hards_in_candidate = HardsInCandidate.objects.filter(
    ...
    russian_text_h2 = ParagraphStyle(
        name="RussianTextH2", fontName="Arial", fontSize=20, leading=20
    )
    styles.add(russian_text_h2)

    russian_text_text = ParagraphStyle(
        name="RussianTextText", fontName="Arial", fontSize=12, leading=12
    )
    styles.add(russian_text_text)

    russian_text_h3 = ParagraphStyle(
        name="RussianTextH3", fontName="Arial", fontSize=18, leading=14
    )
    styles.add(russian_text_h3)

    # Добавление заголовка ФИО кандидата
    styles = getSampleStyleSheet()
    russian_text_style = ParagraphStyle(
        name="RussianText", fontName="Arial", fontSize=22, leading=26
    )
    elements.append(
        Paragraph(
            f"<h1>{candidate.last_name}"
    ...
    elements.append(
        Paragraph(
            f"Телефон: {candidate.contacts_phone};"
    ...
    elements.append(Spacer(1, 6))
    elements.append(
        Paragraph(f"{candidate.specialization.name}", russian_text_text)
    )
    ...
    if course_in_candidate:
        for course in course_in_candidate:
            elements.append(Spacer(1, 6))
            elements.append(
                Paragraph(f"{course.course.name}", russian_text_text)
            )
            elements.append(Spacer(1, 6))
    else:
    ...
    elements.append(Spacer(1, 6))
    elements.append(
        Paragraph(f"{candidate.experience.name}", russian_text_text)
    )
    ...
    if experience_in_candidate:
        for experience in experience_in_candidate:
            ...
            elements.append(
                Paragraph(
                    f"{experience.experience_detailed.date_start} - "
            ...
            elements.append(
                Paragraph(
                    f"{experience.experience_detailed.post}", russian_text_h3
                )
            )
            elements.append(Spacer(1, 6))
            elements.append(
                Paragraph(
                    f"{experience.experience_detailed.responsibilities}",
                    russian_text_text,
                )
            )
            elements.append(Spacer(1, 6))
        ...
        elements.append(
            Paragraph("Опыта работы не указано.", russian_text_text)
        )
    ...
    if education_in_candidate:
        for education in education_in_candidate:
            elements.append(
                Paragraph(
                    f"{education.education.date_start} - "
            ...
            elements.append(
                Paragraph(
                    f"{education.education.education_level}", russian_text_text
                )
            )
            elements.append(Spacer(1, 6))
            ...
            elements.append(
                Paragraph(
                    f"{education.education.name_university}", russian_text_text
                )
            )
        ...
        elements.append(
            Paragraph("Образование не указано.", russian_text_text)
        )
    ...
    with io.open("resume.pdf", "rb") as f:
        response = HttpResponse(
            f.read(), content_type="application/pdf; charset=utf-8"
        )
